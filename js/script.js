...
wonderApp.init = function() {
  $.ajax({
    ...
    data: JSON.stringify({
      ...
      proxyHeaders: {
        Authorization:
          "Basic Njk5ZDkyYjdiY2VmNDljMDkxNGM0ODc0NDEzZWIwOTg6OTExOTU1N2RkOTcxNGFjYWFkNWE2M2NiZTJiMTU5MTk="
      }
    })
  }).then(function(res) {
    headers = {
...
wonderApp.displayAlbumContent = function(info) {
  if (info.preview_url !== null) {
    const container =
      ...
      }" width="200" height="200" alt="" id="artwork" />` +
      `<div id="trackInfos">` +
      `<a href="#" id="${info.id}" onclick="play('audio_${
        info.id
      }')" class="play far fa-play-circle"></a>` +
      `<audio  id="audio_${info.id}" src="${
        info.preview_url
